...
from coauths.explattes.Pesquisador import Pesquisador
...
class Preprocesser:
    ...
    def assign_production_ids(self, df: pd.DataFrame) -> pd.DataFrame:
        df = df.copy()
        df["production"] = df["production"].str.upper()
        unique_productions = sorted(df["production"].unique())
        map_production_to_id = {prod: idx for idx, prod in enumerate(unique_productions)}
        df["production_id"] = df["production"].map(map_production_to_id)
        return df
